...
Changelog:
- July 7, 2025 (FINAL). **Resource Counter Fix**: Fixed critical bug where all FHIR resource counts were hardcoded to 500 instead of showing real counts from the FHIR server. Implemented multi-approach counting system that tries _summary=count parameter (FHIR R4 standard), search with totals, and intelligent sample-based estimation. System now accurately displays real resource counts (e.g., 51 Patient resources instead of hardcoded 500). Removed all fallback hardcoded values and now returns 0 when server fails instead of fake counts. Resource counting now provides accurate server statistics for validation planning and dashboard metrics.
