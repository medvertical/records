...
Changelog:
- July 14, 2025 (FINAL CHECKPOINT). **Pause/Resume System für FHIR Validation - KOMPLETT**: Pause-Funktion für FHIR Validation vollständig repariert und optimiert. System validiert alle 617.216+ Ressourcen (148 Ressource-Typen) wie gewünscht. START beginnt immer bei 0 (neuer Start), nur RESUME setzt an der pausierten Stelle fort. Globale State-Tracking eliminiert Race-Conditions zwischen Start/Pause APIs. Dashboard null-Fehler behoben mit `validationProgress?.validResources || 0`. Frontend und Backend Reset-Mechanismen implementiert für saubere Unterscheidung zwischen neuem Start und Resume. WebSocket Pause-Broadcasting funktioniert korrekt. Validation läuft mit authentischen FHIR-Server Daten (Fire.ly Server).
