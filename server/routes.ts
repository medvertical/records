...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // FHIR Proxy routes - forward FHIR requests to the active FHIR server
  app.get("/api/fhir/:resourceType", async (req, res) => {
    try {
      ...
      console.log(`[FHIR Proxy] GET /${resourceType}`, query);
      
      const bundle = await fhirClient.searchResources(resourceType, query);
      res.json(bundle);
    } catch (error: any) {
      console.error(`[FHIR Proxy] Error fetching ${req.params.resourceType}:`, error.message);
      res.status(500).json({ message: error.message });
    }
  });
  ...
  app.get("/api/fhir/:resourceType/:id", async (req, res) => {
    try {
      ...
      console.log(`[FHIR Proxy] GET /${resourceType}/${id}`);
      
      const resource = await fhirClient.getResource(resourceType, id);
      res.json(resource);
    } catch (error: any) {
      console.error(`[FHIR Proxy] Error fetching ${req.params.resourceType}/${req.params.id}:`, error.message);
      res.status(500).json({ message: error.message });
    }
  });
  ...
  // Bulk validation endpoints - CLEAN VERSION
  app.post("/api/validation/bulk/start", async (req, res) => {
    try {
      ...
      globalValidationState.totalResources = 300; // Mock total for testing
      ...
      console.log('NEW validation start - RESET to 0. Global state:', {
        isRunning: globalValidationState.isRunning,
        canPause: globalValidationState.canPause
      });
      ...
      setImmediate(async () => {
        try {
          console.log('=== DEBUG: Starting background validation ===');
          
          // Simulate processing 300 resources for better visibility
          const totalResourcesToProcess = 300;
          let processed = 0;
          
          for (let i = 0; i < totalResourcesToProcess; i++) {
            ...
            if (processed % 25 === 0) {
              console.log(`=== Progress: ${processed}/${totalResourcesToProcess} resources processed ===`);
          ...
          globalValidationState.shouldStop = false;
          globalValidationState.currentResourceType = null;
          globalValidationState.errors = globalValidationState.errors || [];
          globalValidationState.errors.push({
            level: 'error',
            message: `Critical validation error: ${error?.message || String(error)}`,
            timestamp: new Date().toISOString()
          });
        }
      });
      ...
      console.error('Error starting bulk validation:', error);
      globalValidationState.isRunning = false;
      globalValidationState.isPaused = false;
      globalValidationState.shouldStop = false;
      globalValidationState.currentResourceType = null;
      if (!globalValidationState.errors) {
        globalValidationState.errors = [];
      }
      globalValidationState.errors.push({
        level: 'error',
        message: `Failed to start validation: ${error?.message || String(error)}`,
        timestamp: new Date().toISOString()
      });
      res.status(500).json({ message: error.message });
    }
  ...
  app.get("/api/validation/bulk/progress", async (req, res) => {
    try {
      ...
      const progress = {
        ...
        validResources: globalValidationState.validResources || 0,
        errorResources: globalValidationState.errorResources || 0,
        ...
        processingRate: globalValidationState.processedResources > 0 && globalValidationState.startTime ? 
          globalValidationState.processedResources / ((Date.now() - globalValidationState.startTime.getTime()) / 1000) : null,
  ...
  app.post("/api/validation/bulk/pause", async (req, res) => {
    try {
      ...
      globalValidationState.resumeData = {
        resourceType: globalValidationState.currentResourceType || 'Unknown',
        offset: 0,
        ...
        errors: [],
        startTime: globalValidationState.startTime || new Date()
      };

      console.log('Validation pause requested - setting paused state immediately with resume data:', globalValidationState.resumeData);

      res.json({ 
        message: "Validation paused", 
        status: "paused",
        resumeData: globalValidationState.resumeData
      });
    } catch (error: any) {
      console.error('Error pausing bulk validation:', error);
      res.status(500).json({ message: error.message });
    }
  });

  // Resume bulk validation
  app.post("/api/validation/bulk/resume", async (req, res) => {
    try {
      ...
      console.log('Resuming validation from:', globalValidationState.resumeData);
      globalValidationState.isPaused = false;
      globalValidationState.isRunning = true;
      globalValidationState.shouldStop = false;
      ...
      console.error('Error resuming bulk validation:', error);
      res.status(500).json({ message: error.message });
    }
  });
  ...
  app.post("/api/validation/bulk/stop", async (req, res) => {
    try {
      console.log('Validation stop requested - setting stop state immediately');
      ...
      console.error('Error stopping bulk validation:', error);
      res.status(500).json({ message: error.message });
    }
  });
  ...
  app.post("/api/validation/validate-by-ids", async (req, res) => {
    try {
      ...
      if (resourcesToValidate.length > 0) {
        ...
        try {
          const { cacheManager, CACHE_TAGS } = await import('./utils/cache-manager.js');
