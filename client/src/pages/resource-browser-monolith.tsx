...
export default function ResourceBrowser() {
  ...
  // Track previous settings to detect validation-affecting changes
  ...
  useEffect(() => {
    if (lastChange) {
      console.log('[Settings Change] lastChange detected:', lastChange);
      
      // Get current settings
      const currentSettings = validationSettingsData;
      
      // Check if this is the first time we're seeing settings (initial load)
      if (!previousSettingsRef.current) {
        console.log('[Settings Change] Initial settings load - no revalidation needed');
        previousSettingsRef.current = currentSettings;
        return;
      }
      
      // Check if validation-affecting settings actually changed
      const previousAspects = previousSettingsRef.current?.aspects || {};
      const currentAspects = currentSettings?.aspects || {};
      
      // Compare validation aspects
      const aspectsChanged = JSON.stringify(previousAspects) !== JSON.stringify(currentAspects);
      
      if (aspectsChanged) {
        console.log('[Settings Change] Validation aspects changed - triggering revalidation', {
          previous: previousAspects,
        ...
        console.log('[Settings Change] Non-validation settings changed - no revalidation needed');
      }
      
      // Update previous settings reference
      previousSettingsRef.current = currentSettings;
    }
  }, [lastChange, validationSettingsData, queryClient]);
  ...
  // Track previous page and resource type to detect actual changes
  const previousPageRef = useRef<number | null>(null);
  const previousResourceTypeRef = useRef<string | null>(null);
  
  // Reset validation flag when page or resource type changes
  useEffect(() => {
    const pageChanged = previousPageRef.current !== null && previousPageRef.current !== page;
    const resourceTypeChanged = previousResourceTypeRef.current !== null && previousResourceTypeRef.current !== resourceType;
    
    if (pageChanged || resourceTypeChanged) {
      console.log('[Reset Validation Flag] Resetting due to actual change:', {
        page: { from: previousPageRef.current, to: page, changed: pageChanged },
        resourceType: { from: previousResourceTypeRef.current, to: resourceType, changed: resourceTypeChanged }
      });
      setHasValidatedCurrentPage(false);
    } else {
      console.log('[Reset Validation Flag] No actual change detected - keeping validation state');
    }
  ...
  useEffect(() => {
    console.log('[Auto-Validation Effect] Triggered with:', {
    ...
    if (resourcesData?.resources && resourcesData.resources.length > 0 && !hasValidatedCurrentPage && !isValidating) {
      console.log('[Auto-Validation Effect] Starting validation timer...');
      // Validate unvalidated resources with a short delay to allow UI to render
      const timer = setTimeout(() => {
        console.log('[Auto-Validation Effect] Timer fired - calling validateUnvalidatedResources');
        validateUnvalidatedResources();
      }, 500);
      
      return () => {
        console.log('[Auto-Validation Effect] Cleanup - clearing timer');
        clearTimeout(timer);
      };
    } else {
      console.log('[Auto-Validation Effect] Skipping validation - conditions not met');
    }
